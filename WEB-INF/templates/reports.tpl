...
{if in_array('cl', explode(',', $user->plugins)) || in_array('iv', explode(',', $user->plugins))}          
  ...
  {if in_array('cl', explode(',', $user->plugins))}
                <td width="25%">{$forms.reportForm.chclient.control}&nbsp;{$i18n.label.client}</td>
  {/if}
  {if ($user->canManageTeam() || $user->isClient()) && in_array('iv', explode(',', $user->plugins))}
                <td width="25%">{$forms.reportForm.chinvoice.control}&nbsp;{$i18n.label.invoice}</td>
  {/if}
              </tr>
{/if}            
              <tr>
                <td width="25%">{if ($smarty.const.MODE_PROJECTS == $user->tracking_mode || $smarty.const.MODE_PROJECTS_AND_TASKS == $user->tracking_mode)}{$forms.reportForm.chproject.control}&nbsp;{$i18n.label.project}{/if}</td>
                <td width="25%">{if (($smarty.const.TYPE_START_FINISH == $user->record_type) || ($smarty.const.TYPE_ALL == $user->record_type))}{$forms.reportForm.chstart.control}&nbsp;{$i18n.label.start}{/if}</td>
                <td width="25%">{$forms.reportForm.chduration.control}&nbsp;{$i18n.label.duration}</td>
{if ((($user->canManageTeam() || $user->isClient()) || in_array('ex', explode(',', $user->plugins))) && defined('COST_ON_REPORTS') && isTrue($smarty.const.COST_ON_REPORTS))}
                  <td width="25%">{$forms.reportForm.chcost.control}&nbsp;{$i18n.label.cost}</td>
{else}
                  <td></td>
{/if}
              </tr>
              <tr>
              	<td>{if ($smarty.const.MODE_PROJECTS_AND_TASKS == $user->tracking_mode)}{$forms.reportForm.chtask.control}&nbsp;{$i18n.label.task}{/if}</td>
              	<td>{if (($smarty.const.TYPE_START_FINISH == $user->record_type) || ($smarty.const.TYPE_ALL == $user->record_type))}{$forms.reportForm.chfinish.control}&nbsp;{$i18n.label.finish}{/if}</td>
                <td>{$forms.reportForm.chnote.control}&nbsp;{$i18n.label.note}</td>
{if ($custom_fields && $custom_fields->fields[0])}
                <td>{$forms.reportForm.chcf_1.control}&nbsp;{$custom_fields->fields[0]['label']|escape:'html'}</td>
{else}
                <td></td>
{/if}
              </tr>
        ...
        <tr valign="top">
          <td>{$forms.reportForm.group_by.control} {$forms.reportForm.chtotalsonly.control} {$i18n.form.reports.totals_only}</td>
        </tr>
      </table>
